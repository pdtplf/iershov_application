...
- Добавить генерацию названия почтового адреса (для безопасности)
...
- У компания Apple имеется сервис "Hide My Email", в нем реализованы функции переадресации писем и меню управлением всеми письмами, но это решение доступно только при подписке iCloud+. (Существует возможность использовать этот сервис бесплатно, но накладываются ограничения в виде: 1. обязательного наличия устройства Apple 2. возможность использовать сервис только в приложениях где имеется вход через Apple ID)
- Существуют такие временные сервисы которые доступные из веб-сайта, их очень много, поэтому я разобью на две категории. 
	...
	2. Более долговечные. Эти сервисы предлагают пользователю самому выбрать адрес на который они будут получать письма. Но письма приходящие на эти ящики так же хранятся совсем недолго (1-7 дней) и письма доступны для просмотра всем кто зашел на этот сайт, что совсем не безопасно.
